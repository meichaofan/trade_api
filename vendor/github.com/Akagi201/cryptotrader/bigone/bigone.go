...
func (c *Client) GetTicker(ctx context.Context, quote string, base string) (*model.Ticker, error) {
	...
	buyRes := gjson.GetBytes(body, "data.bids").Array()
	buy, err := strconv.ParseFloat(buyRes[0].Get("price").String(), 64)
	if err != nil {
		return nil, err
	}
	sellRes := gjson.GetBytes(body, "data.asks").Array()
	sell, err := strconv.ParseFloat(sellRes[0].Get("price").String(), 64)
	if err != nil {
		return nil, err
	}
	closeRes1 := gjson.GetBytes(body, "data.metrics.0000060").Array()
	close1, err := strconv.ParseFloat(closeRes1[0].Array()[4].String(), 64)
	open1, err := strconv.ParseFloat(closeRes1[0].Array()[1].String(), 64)
	fmt.Println("close:",close1)
	if err != nil {
		return nil, err
	}
	precent24 := (clos - open) / open
	precent1 := (close1 - open1) / open1
	return &model.Ticker{
		Last:  last,
		Low:   low,
		High:  high,
		...
		PriceChangePercent1h:precent1,

	}, nil
}

func (c *Client) GetRecords(ctx context.Context, base string, quote string,typ string) ([]model.Record, error) {
	url := API +  strings.ToUpper(quote) + "-" + strings.ToUpper(base)
	...
	getpath := "data.metrics." + typ
	var records []model.Record
	var recordscache []model.Record
	//datajson := gjson.GetBytes(body, "data.metrics.0000001")
	datajson := gjson.GetBytes(body, getpath)
	//fmt.Println(string(body))
	//fmt.Println(getpath)
	//fmt.Println(string(datajson.String()))
	if datajson.Exists() {
		re := datajson.Array()
		if typ == "1w" {
			//fmt.Println(len(re))
			...
			for _,v:=  range re {
				//fmt.Println(v.Array()[0].Int()/1000)
				record := model.Record{
