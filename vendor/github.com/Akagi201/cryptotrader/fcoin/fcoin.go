...
func New(accessKey string, secretKey string) *Fcoin {
	return &Fcoin{
		AccessKey: accessKey,
		SecretKey: secretKey,
	}
...
// GetTicker 行情
func (hb *Fcoin) GetTicker(base string, quote string) (*model.Ticker, error) {
...
func (z *Fcoin) GetRecords(base string, quote string, typ string, since int, size int) ([]model.Record, error) {
...
func (c *Fcoin) GetTrades(quote string, base string) ([]model.Trade, error) {
...
func (c *Fcoin) GetDeep(quote string, base string) (model.OrderBook, error) {
...
func (c *Fcoin) GetMarkets() ([]model.MarketPairInfo, error) {
	...
	if datajson.Exists() {
		...
		for _, v2 := range re {
			trade := model.MarketPairInfo{
				Quote: v2.Get("base_currency").String(),
				Base:  v2.Get("quote_currency").String(),
			}
			records = append(records, trade)
