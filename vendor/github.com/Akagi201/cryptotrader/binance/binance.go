...
const (
	...
	ApiV1    = "v1"
	ApiV3    = "v3"
	ORDERURL = "https://api.binance.com/api/v1/depth?"
)
...
func (c *Client) GetTicker(ctx context.Context, quote string, base string) (*model.Ticker, error) {
	...
	pricechangepercentRes := gjson.GetBytes(body, "priceChangePercent").String()
	pricechangepercent, err := strconv.ParseFloat(pricechangepercentRes, 64)
	if err != nil {
		return nil, err
	}
	return &model.Ticker{
...
func (c *Client) GetTrades(ctx context.Context, quote string, base string, fromID int64, startTime int64, endTime int64, limit int64) ([]model.Trade, error) {
	...
	gjson.ParseBytes(body).ForEach(func(key, value gjson.Result) bool {
		...
		trade.TradeTime = value.Get("T").Int()
		if value.Get("m").String() == "true" {
			trade.Type = "buy"
		} else {
			trade.Type = "sell"
		}
...
type DeepDataResp struct {
	...
	Bids [][]interface{} `json:"bids" bson:"bids"`
	Asks [][]interface{} `json:"asks" bson:"asks"`
	Id int `json:"lastUpdateId" bson:"lastUpdateId"`
...
func (c *Client) GetDeep(base string, quote string) (model.OrderBook, error) {
	...
	url := ORDERURL + "symbol=" + strings.ToUpper(quote) + strings.ToUpper(base) +"&limit=1000"
	...
	records.OrderTime = timestamp
	for _,v := range transferdata.Asks {
		asks_price := v[0].(string)
		asks_amount := v[1].(string)
		pricefloat64, _ := strconv.ParseFloat(asks_price, 64)
		amountfloat64, _ := strconv.ParseFloat(asks_amount, 64)
